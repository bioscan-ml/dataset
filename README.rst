...
The package source code is available on `GitHub <GitHub repository_>`_.
...
.. code-block:: python
    ...
    dataset = CanadianInvertebrates(root="~/Datasets/bioscan/")
...
For example, to load only the latitude and longitude coordinates as inputs:

.. code-block:: python

    dataset = BIOSCAN5M(root="~/Datasets/bioscan/", modality=("coord-lat", "coord-lon"))

or to load the size of the insect (in pixels) in addition to the image and DNA barcode:

.. code-block:: python

    dataset = BIOSCAN5M(
        root="~/Datasets/bioscan/", modality=("image", "dna", "image_measurement_value")
    )
...
The `DNA BIN <what-is-DNA-BIN_>`_ is similar in granularity to the species, but was generated by clustering the DNA barcodes instead of by inspecting their morphology.
...
For example, this code will load the BIOSCAN-5M dataset with a transform that resizes the image to 256x256 pixels and normalizes the pixel values, and applies a character-level tokenizer to the DNA barcode with padding to 660 base pairs.
...
In this second example, we apply a transform to the taxonomic labels to convert them to a single string.
...
If you find this package useful in your research, please consider giving our `GitHub repository`_ a star ⭐.
...
.. code-block:: bibtex
    ...
    ﻿@article{CanadianInvertebrates,
        title={A reference library for {C}anadian invertebrates with 1.5 million barcodes, voucher specimens, and {DNA} samples},
        author={deWaard, Jeremy R. and Ratnasingham, Sujeevan and Zakharov, Evgeny V.
            and Borisenko, Alex V. and Steinke, Dirk and Telfer, Angela C.
            and Perez, Kate H. J. and Sones, Jayme E. and Young, Monica R.
            and Levesque-Beaudin, Valerie and Sobel, Crystal N. and Abrahamyan, Arusyak
            and Bessonov, Kyrylo and Blagoev, Gergin and deWaard, Stephanie L.
            and Ho, Chris and Ivanova, Natalia V. and Layton, Kara K. S.
            and Lu, Liuqiong and Manjunath, Ramya and McKeown, Jaclyn T. A.
            and Milton, Megan A. and Miskie, Renee and Monkhouse, Norm
            and Naik, Suresh and Nikolova, Nadya and Pentinsaari, Mikko
            and Prosser, Sean W. J. and Radulovici, Adriana E. and Steinke, Claudia
            and Warne, Connor P. and Hebert, Paul D. N.
        },
...
.. _GitHub repository: https://github.com/bioscan-ml/dataset
